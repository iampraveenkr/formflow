...
export default function WorkflowDetailPage({ params }: WorkflowPageProps): JSX.Element {
  ...
  async function handleRunCheck(): Promise<void> {
    ...
    if (!response.ok) {
      const body = (await response.json()) as { error?: string };
      setErrorMessage(body.error ?? "Unable to evaluate workflow run.");
      return;
    }
  ...
  async function handleSync(): Promise<void> {
    ...
    const payload = body.data ?? (body.form && body.fields ? { form: body.form, fields: body.fields } : null);
    if (!payload) {
      setErrorMessage("Sync succeeded but returned an unexpected payload.");
      return;
    }

    setFormId(payload.form.id);
    setRawSchema(payload.form.schemaJson);
