...
test("form lookup is workspace scoped", async () => {
  await upsertFormCache({
    id: "form_ws_scoped",
    workspaceId: "ws_scope_a",
    googleFormId: "g_scope_1",
    title: "Scoped Form",
    schemaJson: {},
    schemaVersionHash: "scoped",
    lastSyncedAt: new Date().toISOString()
  });

  const inScope = await findFormByIdWithinWorkspace("ws_scope_a", "form_ws_scoped");
  const outOfScope = await findFormByIdWithinWorkspace("ws_scope_b", "form_ws_scoped");

  assert.equal(inScope?.id, "form_ws_scoped");
